...
export default function Edit() {
	...
	const { flash, errors, brand }: { flash?: { success?: string; error?: string }; errors?: Record<string, string>; brand: any } = usePage().props;
	const [logoPreview, setLogoPreview] = useState<string | null>(brand?.logo ? brand.logo_url : null);
	...
	function onSubmit(values: any) {
		...
		router.post(`/admin/brands/${brand.id}`, data, {
			forceFormData: true,
			onSuccess: () => {
				toast({ title: 'Marca atualizada', description: 'A marca foi atualizada com sucesso.', variant: 'success' });
			},
			onError: () => {
				toast({ title: 'Erro ao atualizar', description: 'Verifique os erros no formulário.', variant: 'destructive' });
			},
		});
	}
