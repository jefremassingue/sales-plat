...
const formSchema = z.object({
  ...
  status: z.enum(['draft', 'pending', 'paid', 'partial', 'canceled', 'overdue']),
...
interface Props {
  ...
  products: { id: number; name: string; price: number; description?: string; unit?: string; category?: { id: number; name: string } }[];
  warehouses: { id: number; name: string }[];
  currencies: { code: string; name: string; symbol: string; exchange_rate: number; decimal_separator: string; thousand_separator: string; decimal_places: number }[];
  defaultCurrency: { code: string; name: string; symbol: string; exchange_rate: number; decimal_separator: string; thousand_separator: string; decimal_places: number };
  taxRates: { id: number; name: string; value: number; is_default: boolean }[];
  units: { value: string; label: string }[];
  statuses: { value: string; label: string; color: string }[];
  paymentMethods: { value: string; label: string }[];
  quotation?: { id: number; quotation_number: string; customer_id: number; notes?: string; terms?: string; items: any[] };
}
...
export default function Edit({
  ...
  const [itemBeingEdited, setItemBeingEdited] = useState<FormValues['items'][number] & { index: number } | null>(null);
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: sale ? {
      sale_number: sale.sale_number ?? salePlaceholder,
      customer_id: sale.customer_id?.toString() ?? '',
      issue_date: new Date(sale.issue_date),
      due_date: sale.due_date ? new Date(sale.due_date) : null,
      status: sale.status,
      currency_code: sale.currency_code,
      exchange_rate: sale.exchange_rate.toString(),
      include_tax: sale.include_tax,
      notes: sale.notes ?? '',
      terms: sale.terms ?? '',
      shipping_amount: sale.shipping_amount?.toString() ?? '0',
      shipping_address: sale.shipping_address ?? '',
      billing_address: sale.billing_address ?? '',
      payment_method: sale.payment_method ?? (paymentMethods.length > 0 ? paymentMethods[0].value : ''),
      amount_paid: sale.amount_paid?.toString() ?? '0',
      reference: sale.reference ?? '',
      items: sale.items.map(item => ({
        id: item.id,
        product_id: item.product_id ? item.product_id.toString() : null,
        product_variant_id: item.product_variant_id ? item.product_variant_id.toString() : null,
        warehouse_id: item.warehouse_id ? item.warehouse_id.toString() : null,
        name: item.name,
  ...
  const { fields, append, remove, update } = useFieldArray({
    ...
    name: "items",
    keyName: "formId", // Add keyName for better item tracking
  });
    ...
    if (errors) {
      Object.keys(errors).forEach(key => {
        if (key.startsWith('items.')) {
          ...
          form.setError(`items.${index}.${field as keyof FormValues['items'][number]}`, {
            type: 'manual',
            message: errors[key],
          });
        } else {
  ...
  const handleUpdateItem = async (index: number, field: string, value: string) => {
    const currentItem = { ...fields[index] } as FormValues['items'][number];
  ...
  const handleSaveItemEdit = (editedItem: any) => {
    ...
    const { index: _idx, ...itemToUpdate } = editedItem;
  ...
  const handleAddManualItem = (item: any) => {
    append({
      ...
      warehouse_id: item.warehouse_id ?? selectedWarehouseId, // Use selectedWarehouseId if not provided
      description: item.description ?? '',
      unit: item.unit ?? 'unit',
      discount_percentage: item.discount_percentage ?? '0',
      tax_percentage: item.tax_percentage ?? (taxRates.find(tax => tax.is_default == true)?.value.toString() || '16'),
    });
  };
  ...
  const handleSubmit = () => {
    ...
    form.handleSubmit((values: FormValues) => {
      ...
      try {
        ...
        if (sale) {
          router.put(`/admin/sales/${sale.id}`, data, {
            onSuccess: () => {
  ...
  const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: sale ? `Editar Venda ${sale.sale_number ?? ''}` : 'Nova Venda', href: sale ? `/admin/sales/${sale.id}/edit` : '/admin/sales/create' },
  ];

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={sale ? `Editar Venda ${sale.sale_number ?? ''}` : "Ponto de Venda - Nova Venda"} />
      ...
      <div className="container px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">{sale ? `Editar Venda ${sale.sale_number}` : "Ponto de Venda - Nova Venda"}</h1>
              {quotation && (
